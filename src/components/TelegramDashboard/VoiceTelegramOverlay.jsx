...
import '../WhatsAppDashboard/VoiceWhatsAppOverlay.css'; // Just reuse the same css structural file to map classes
import '../VoiceAssistant/AutoListeningIndicator.css';

const TRANSLATIONS = {
    'en-US': {
        cmds: {
            compose: { phrases: ['send telegram', 'send message', 'telegram', 'compose message', 'compose', 'new message'], reply: "Preparing Telegram message." },
            ...
            help: { phrases: ['help'], reply: "Say 'Send Telegram to' followed by a contact name, or 'Read latest messages'." },
        ...
        no_api: "Telegram API is not configured yet.",
        no_recent: "You have no recent Telegram conversations.",
        ...
        did_not_understand: "I didn't understand. You can say 'Send Telegram'.",
    ...
    'es-ES': {
        cmds: {
            compose: { phrases: ['enviar telegram', 'enviar mensaje', 'telegram', 'nuevo mensaje'], reply: "Preparando mensaje de Telegram." },
            ...
            help: { phrases: ['ayuda'], reply: "Di 'Enviar Telegram a' seguido del nombre del contacto, o 'Leer últimos mensajes'." },
        ...
        no_api: "La API de Telegram no está configurada aún.",
        no_recent: "No tienes conversaciones recientes de Telegram.",
        ...
        did_not_understand: "No entendí. Puedes decir 'Enviar Telegram'.",
    ...
    'fr-FR': {
        cmds: {
            compose: { phrases: ['envoyer telegram', 'envoyer un message', 'telegram', 'nouveau message'], reply: "Préparation du message Telegram." },
            ...
            help: { phrases: ['aide'], reply: "Dites 'Envoyer Telegram à' suivi d'un nom de contact, ou 'Lire les derniers messages'." },
        ...
        no_api: "L'API Telegram n'est pas encore configurée.",
        no_recent: "Vous n'avez pas de conversations Telegram récentes.",
        ...
        did_not_understand: "Je n'ai pas compris. Vous pouvez dire 'Envoyer Telegram'.",
...
const VoiceTelegramOverlay = ({ user, onLogout, configured, onGoApps, onSwitchEmail, onCompose, onAddContact, onEditContact, onDeleteContact, onStarContact, onOpenChat, onSendVoiceMessage, contacts = [], conversations = [], activeContact = null, messages = [], autoListen = true, isVoiceComposeActive = false, isMuted = false, setIsMuted }) => {
    const { lang } = useLang();
    ...
    return (
        <div className="voice-overlay-container" style={{ pointerEvents: 'none', zIndex: 9999 }}>
            <div className={`glass-panel voice-panel ${autoListeningEnabled ? 'auto-listening-mode' : ''}`} style={{ pointerEvents: 'auto', position: 'fixed', bottom: '2rem', right: '2rem', transition: 'all 0.3s ease', borderTop: '2px solid rgba(42, 171, 238, 0.5)' }}>

                <div className="voice-visualizer">
                    <div className={`orb orb--${assistantState}`} style={assistantState === 'listening' ? { background: 'radial-gradient(circle, #2AABEE 0%, transparent 70%)', boxShadow: '0 0 40px #2AABEE' } : {}}>
                        <div className="orb-core" style={{ background: '#2AABEE', boxShadow: '0 0 20px #2AABEE' }} />
                        <div className="orb-ring ring-1" style={{ borderColor: '#2AABEE' }} />
                        <div className="orb-ring ring-2" style={{ borderColor: '#1c8dcb' }} />
                    </div>
                    <p className="orb-state-label" style={{ color: '#2AABEE' }}>
                ...
                <div className="va-mic-section">
                    <button
                        ...
                        style={isListening ? { background: '#2AABEE', color: '#000' } : {}}
                    ...
                    <button className="primary-btn" onClick={() => onCompose && onCompose()} style={{ background: 'rgba(42, 171, 238, 0.15)', color: '#2AABEE', border: '1px solid rgba(42, 171, 238, 0.3)', flex: 1, padding: '10px' }}>
                        Voice Compose
                    </button>
                </div>
                ...
                {assistantState === 'idle' && (
                    <div className="demo-commands">
                        ...
                        <div className="demo-btns">
                            {['Send Telegram message', 'Read latest message', 'Read messages from...', 'Switch to Email'].map(cmd => (
                                <button key={cmd} className="demo-cmd" onClick={() => processCommand(cmd)} style={{ borderColor: 'rgba(42, 171, 238, 0.3)' }}>{cmd}</button>
                            ))}
                        </div>
                    </div>
                )}
...
export default VoiceTelegramOverlay;
